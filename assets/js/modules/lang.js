// ================================================
// Translation Dictionary (English / Arabic)
// Hero, About, Skills, Projects, Contact & Footer
// ================================================
export const translations = {
  en: {
    // ===== HERO =====
    hero_title: "Hi, I'm Orwa Mayasah",
    hero_subtitle: "Front-End Developer & UI Enthusiast",
    hero_desc:
      "I build fast, accessible, and visually engaging web experiences focused on performance and user delight.",
    btn_projects: "View Projects",
    btn_contact: "Contact Me",

    // ===== TOOLTIP =====
    tooltip_home: "Home",
    tooltip_about: "About",
    tooltip_skills: "Skills",
    tooltip_projects: "Projects",
    tooltip_contact: "Contact",

    // ===== ABOUT =====
    about_title: "About Me",
    about_desc:
      "I'm a passionate Front-End Developer focused on building real projects and improving my skills every day.",
    about_role_label: "Role",
    about_role_value: "Front-End Developer",
    about_exp_label: "Experience",
    about_exp_value: "Learning through applied work",
    about_location_label: "Location",
    about_location_value: "Syria • Remote or On-Site",
    about_lang_label: "Languages",
    about_lang_value: "Arabic • English",

    // ===== SKILLS =====
    skills_title: "Skills",
    skills_subtitle:
      "A focused set of front-end skills I continuously evolve through real projects.",
    skills_group1: "Core Front-End",
    skills_group2: "Tools & Libraries",
    skills_group3: "React Ecosystem",
    skills_group4: "Additional Skills",

    // ===== PROJECTS =====
    projects_title: "Featured Projects",
    projects_subtitle:
      "High-quality, interactive web experiences built with modern front-end technologies.",
    projects_card1_title: "Orwa Tech Website",
    projects_card1_desc:
      "A modern, fully responsive tech website featuring clean UI, animation, and reusable components.",
    projects_card1_btn_live: "Live Demo",
    projects_card1_btn_github: "GitHub",
    projects_card2_title: "Agency Website",
    projects_card2_desc:
      "A sleek digital agency website using advanced layouts, smooth animations, and modern responsive design techniques.",
    projects_card2_btn_live: "Live Demo",
    projects_card2_btn_github: "GitHub",

    // ===== CONTACT =====
    contact_title: "Contact",
    contact_subtitle: "Feel free to reach out for collaboration!",
    contact_name_label: "Name",
    contact_name_placeholder: "Please enter your name…",
    contact_email_label: "Email",
    contact_email_placeholder: "Please enter your email…",
    contact_subject_label: "Subject",
    contact_subject_placeholder: "Please enter the subject…",
    contact_message_label: "Message",
    contact_message_placeholder: "Please enter your message…",
    btn_send: "Send Message",
    contact_location: "Syria • Remote or On-Site",

    // ===== FOOTER =====
    footer_title: "Orwa Mayaseh",
    footer_desc: "Front-End Developer — Building modern, fast and clean web interfaces.",
    footer_quick_links: "Quick Links",
    footer_link_projects: "Projects",
    footer_link_about: "About Me",
    footer_link_services: "Services",
    footer_link_contact: "Contact",
    footer_socials: "Social Media",
    footer_social_link_linkedin: "LinkedIn",
    footer_social_link_codepen: "CodePen",
    footer_social_link_github: "GitHub",
    footer_social_link_facebook: "Facebook",
    footer_social_link_telegram: "Telegram",
    footer_social_link_instagram: "Instagram",
    footer_social_link_x: "Twitter / X",
  },

  ar: {
    // ===== HERO =====
    hero_title: "مرحبًا، أنا عروة مياسة",
    hero_subtitle: "مطوّر واجهات أمامية وشغوف بتجربة المستخدم",
    hero_desc:
      "أصمّم تجارب ويب سريعة وسهلة الوصول وجذابة بصريًا، مع تركيز على الأداء وتجربة المستخدم.",
    btn_projects: "عرض المشاريع",
    btn_contact: "تواصل معي",

    // ===== TOOLTIP =====
    tooltip_home: "الرئيسية",
    tooltip_about: "عني",
    tooltip_skills: "المهارات",
    tooltip_projects: "المشاريع",
    tooltip_contact: "تواصل",

    // ===== ABOUT =====
    about_title: "عنيّ",
    about_desc:
      "أنا مطوّر واجهات أمامية شغوف ببناء مشاريع حقيقية وتطوير مهاراتي يومًا بعد يوم.",
    about_role_label: "الدور",
    about_role_value: "مطوّر واجهات أمامية",
    about_exp_label: "الخبرة",
    about_exp_value: "أتطور عبر العمل التطبيقي",
    about_location_label: "الموقع",
    about_location_value: "سوريا • العمل عن بُعد أو في الموقع",
    about_lang_label: "اللغات",
    about_lang_value: "العربية • الإنجليزية",

    // ===== SKILLS =====
    skills_title: "المهارات",
    skills_subtitle:
      "مجموعة مركّزة من مهارات الواجهة الأمامية أطورها باستمرار من خلال المشاريع العملية.",
    skills_group1: "أساسيات الواجهة الأمامية",
    skills_group2: "الأدوات والمكتبات",
    skills_group3: "نظام React",
    skills_group4: "مهارات إضافية",

    // ===== PROJECTS =====
    projects_title: "أبرز المشاريع",
    projects_subtitle:
      "تجارب ويب تفاعلية عالية الجودة مبنية باستخدام أحدث تقنيات الواجهة الأمامية.",
    projects_card1_title: "موقع Orwa Tech",
    projects_card1_desc:
      "موقع تقني حديث ومتجاوب بالكامل، يتميز بتصميم نظيف، حركات سلسة، ومكوّنات قابلة لإعادة الاستخدام.",
    projects_card1_btn_live: "عرض مباشر",
    projects_card1_btn_github: "GitHub",
    projects_card2_title: "موقع الوكالة Agency",
    projects_card2_desc:
      "موقع وكالة رقمية أنيق يعتمد على تخطيطات متقدمة، وحركات سلسة، وتصميم متجاوب عصري.",
    projects_card2_btn_live: "عرض مباشر",
    projects_card2_btn_github: "GitHub",

    // ===== CONTACT =====
    contact_title: "تواصل",
    contact_subtitle: "لا تتردد بالتواصل للعمل أو التعاون!",
    contact_name_label: "الاسم",
    contact_name_placeholder: "أدخل اسمك…",
    contact_email_label: "البريد الإلكتروني",
    contact_email_placeholder: "أدخل بريدك الإلكتروني…",
    contact_subject_label: "الموضوع",
    contact_subject_placeholder: "أدخل موضوع الرسالة…",
    contact_message_label: "الرسالة",
    contact_message_placeholder: "اكتب رسالتك هنا…",
    btn_send: "إرسال الرسالة",
    contact_location: "سوريا • العمل عن بُعد أو في الموقع",

    // ===== FOOTER =====
    footer_title: "عروه مياسه",
    footer_desc: "مطور واجهة أمامية — بناء واجهات ويب حديثة وسريعة ونظيفة.",
    footer_quick_links: "روابط سريعة",
    footer_link_projects: "المشاريع",
    footer_link_about: "عني",
    footer_link_services: "الخدمات",
    footer_link_contact: "تواصل",
    footer_socials: "وسائل التواصل",
    footer_social_link_linkedin: "لينكدإن",
    footer_social_link_codepen: "كود بن",
    footer_social_link_github: "جيت هب",
    footer_social_link_facebook: "فيسبوك",
    footer_social_link_telegram: "تيليجرام",
    footer_social_link_instagram: "إنستغرام",
    footer_social_link_x: "تويتر / X",
  },
};
